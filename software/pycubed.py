...
class Satellite:
    def __init__(self):
        ...
        # Define burn wires:
        self._relayA = digitalio.DigitalInOut(board.D2) #CHANGED
        self._relayA.switch_to_output(drive_mode=digitalio.DriveMode.OPEN_DRAIN)
        self._deployA = False

        # Define battery voltage
        self._vbatt = analogio.AnalogIn(board.A3) #CHANGED

        # Define MPPT charge current measurement
        self._ichrg = analogio.AnalogIn(board.A4) #CHANGED
        ...
        # Define GPS
        self.en_gps = digitalio.DigitalInOut(board.D3) #CHANGED
        self.en_gps.switch_to_output()

        # Define sdcard
        self._sdcs = digitalio.DigitalInOut(board.D4) #CHANGED
        self._sdcs.switch_to_output(value=True)

        # Define radio
        self._rf_cs1 = digitalio.DigitalInOut(board.D5)#CHANGED
        self._rf_rst1 = digitalio.DigitalInOut(board.D6) #CHANGED
        self._rf_cs1.switch_to_output(value=True)
        self._rf_rst1.switch_to_output(value=True)
        self._rf_cs2 = digitalio.DigitalInOut(board.D7) #CHANGED
        self._rf_rst2 = digitalio.DigitalInOut(board.D8) #CHANGED
